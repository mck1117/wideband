...
static const SPIConfig spi_config[EGT_CHANNELS] = {{.circular = false,
                                                    .slave = false,
                                                    .data_cb = nullptr,
                                                    .error_cb = nullptr,
                                                    .ssport = EGT_CS0_PORT,
...
int Max3185x::spi_rx32(uint32_t* data)
{
    ...
    if (data)
    {
        *data = (rx[0] << 24) | (rx[1] << 16) | (rx[2] << 8) | (rx[3] << 0);
    }
    return 0;
}
...
Max3185xType Max3185x::detect()
{
    ...
    uint32_t data = (rx[0] << 24) | (rx[1] << 16) | (rx[2] << 8) | (rx[3] << 0);
...
#define MAX33855_VCC_BIT BIT(2)
    ...
    if ((ret) || ((data & MAX31855_RESERVED_BITS) != 0) || (data == 0x0) || (data == 0xffffffff))
    {
...
template <> const livedata_egt_s* getLiveData(size_t ch)
{
    if (ch < EGT_CHANNELS)
    {
...
template <> const livedata_egt_s* getLiveData(size_t)
{
    return nullptr;
}
